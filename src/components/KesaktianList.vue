<template>
  <div class="p-4">
    ...
    <h1 class="text-2xl font-bold mb-4">Daftar Kesaktian</h1>
    <div v-if="loading">Memuat...</div>
    <div v-else-if="error">{{ error }}</div>
    <div v-else>
      <ul class="list-disc ml-4">
        <li v-for="item in kesaktianList" :key="item">
  ...
  mounted() {
    ...
    axios
      ...
      .then(response => {
        ...
        this.kesaktianList = results
          .map(r => r.individu.value.split(/[#/]/).pop())
          .sort((a, b) => a.localeCompare(b))
      })
      .catch(error => {
        this.error = 'Gagal memuat data kesaktian.'
        console.error(error)
      })
      .finally(() => {
        this.loading = false
